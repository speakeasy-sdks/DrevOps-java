/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package DrevOps.CircleCi.models.operations;

import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * GetPipelineByNumberPipelineTriggerActor - The user who triggered the Pipeline.
 */
public class GetPipelineByNumberPipelineTriggerActor {
    /**
     * URL to the user's avatar on the VCS
     */
    @JsonProperty("avatar_url")
    public String avatarUrl;

    public GetPipelineByNumberPipelineTriggerActor withAvatarUrl(String avatarUrl) {
        this.avatarUrl = avatarUrl;
        return this;
    }
    
    /**
     * The login information for the user on the VCS.
     */
    @JsonProperty("login")
    public String login;

    public GetPipelineByNumberPipelineTriggerActor withLogin(String login) {
        this.login = login;
        return this;
    }
    
    public GetPipelineByNumberPipelineTriggerActor(@JsonProperty("avatar_url") String avatarUrl, @JsonProperty("login") String login) {
        this.avatarUrl = avatarUrl;
        this.login = login;
  }
}
